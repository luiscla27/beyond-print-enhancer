class DomManager {
  constructor() {
    ...
    this.selectors = {
      CORE: {
        ...
        TIDBITS_NAME: ".ddbc-character-tidbits__heading h1",
      ...
      EXTRACTABLE: {
        ...
        HEADER_GENERIC:
          'h1, h2, h3, h4, h5, [class*="head"], [data-testid*="header"], [data-testid*="heading"]',
  ...
  getNavigation() {
    ...
    const el =
      ...
      document.querySelector(this.selectors.CORE.NAVIGATION);
    return new ElementWrapper(el);
  }
  ...
  getSpellRows(context = document) {
    ...
    const els = context.querySelectorAll
      ? context.querySelectorAll(this.selectors.SPELLS.ROW)
      : document.querySelectorAll(this.selectors.SPELLS.ROW);
    return Array.from(els).map((el) => new ElementWrapper(el));
  }
