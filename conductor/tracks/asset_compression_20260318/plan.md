...
- [x] Update `ASSET_METADATA` in `js/main.js` with new slice values and `.webp` extensions.
- [x] Update `ASSET_LIST` in `js/main.js`.
- [x] Update `manifest.json` web_accessible_resources to include `*.webp`.
- [x] Global search and replace `.gif` with `.webp` in `js/main.js`, `js/spells.js`, and `test/**/*.js`.

## Phase 4: Verification
- [x] Verify total file size reduction.
- [x] Run all unit tests.
- [x] Manual verification of border rendering (via simulated tests).
- [x] Remove old `.gif` files.
