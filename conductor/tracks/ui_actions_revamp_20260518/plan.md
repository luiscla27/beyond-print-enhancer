...
- [ ] Task: Write tests verifying that sections only display `be-select-section-button` and `be-clone-delete` natively, with others inside the menu.
- [ ] Task: Refactor section UI injection logic to append `be-clone-button`, `be-border-button`, `be-split-skills-button`, and `be-compact-button` to the new context menu.
...
- [ ] Task: Write tests verifying that shapes only display `be-shape-delete` and `be-shape-rotate` natively, with others inside the menu.
- [ ] Task: Refactor shape UI injection logic to append `be-shape-switch` and `be-shape-clone` to the new context menu.
- [ ] Task: Ensure placement and styling of the context menu button for shapes is completely consistent with sections.
