...
- **Layout Optimization & Interaction:**
    - **Headerless Dragging:** Eliminates cluttered drag handles and minimize buttons. Sections and shapes are draggable by clicking directly on their background areas, with intuitive "grab" cursors providing immediate visual feedback of draggability.
    - **Advanced CSS Hooks:** Section wrappers include specific CSS hooks (e.g., `.be-section-primary-box`) for advanced styling and layout exceptions.
    - **Clean UI:** Removes site headers, navigation bars, sidebars, and interactive buttons (e.g., Short/Long Rest, Manage Spells) to maximize printable space.
- **Persistent Layout Management:** Provides a floating control panel to save custom section arrangements (coordinates, sizes, and internal scaling) to the browser (IndexedDB) or as local JSON files for portability. Includes automatic migration of legacy data and support for embedded custom assets. Elements are identifiable via an interactive Layer Manager that synchronizes with the custom titles assigned to each section.
...
- **Skill Box Splitting:** Provides a specialized "Splitter" button for the combined skills section, allowing users to decompose it into 5 separate, stat-linked sections (STR, INT, WIS, CHA, DEX). Each split section automatically filters out unrelated skills, enabling users to organize their skills alongside their respective ability scores.
...
- **Decorative Shapes:** Enables users to add independent, floating, and resizable decorative elements (shapes/borders) to the layout. Includes an enhanced 'Add Shape' modal with categorized tabs and tag-based filtering for a large library of assets. Supports **user-uploaded custom assets** from disk, with automatic Base64 encoding and global persistence.
- **Custom Asset Management:** Allows users to upload personal image files (PNG, JPEG, WebP, SVG) to be used as shapes. Large files are automatically compressed to WebP format to maintain layout portability and performance.
...
- **Shape Layer Management:** Advanced multi-layer system for decorative shapes, allowing users to create, reorder, and group assets into unlimited layers (with a 100-element limit per layer for optimized print z-index management). Includes nested visual lists with thumbnails and previews to help users identify layer contents.
...
- **Template Catalog:** Provides a library of professional, themed layouts (e.g., 'Archer Template') that can be applied with a single click, automatically configuring borders and adding decorative shapes.
...
- **Centralized Properties Panel:** Introduces a dedicated management panel in the main control panel for granular control over individual sections. When a section is marked as "Active" (via the '🎯' button), users can adjust its specific attributes (Pixel-based Font Size, Compact Mode, and Border Style) in real-time. This provides a persistent alternative to floating modals and facilitates rapid layout fine-tuning.
