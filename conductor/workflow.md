...
- [ ] **Load Default Handling:** Ensure any new kind of section (clones, detail sections, extractions, etc.) is correctly repositioned or handled in `handleLoadDefault` (update `handleLoadDefault` in `main.js`). Note: Spell detail sections should default to `left: 1200px` (outside the main sheet limit), while extractions should be rolled back (original element shown, floating section removed).
