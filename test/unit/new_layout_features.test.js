...
window.eval(sectionUtilsContent);
  ...
  describe('Control Panel UI', function() {
      it('should create a vertical control panel in the top-left', function() {
          ...
          // AC-3 (first_run_and_panel_20260911): this used to assert a 'Contribute' button in the
          // panel. It is not weakened to "some button exists" — it is INVERTED, because the move is
          // the change: the panel must carry no fundraising row, and the destination must still be
          // reachable from the action-icon menu (pinned in ui_symmetry.test.js and by a browser case).
          assert.ok(
            !labels.some(l => l.includes('Contribute')),
            'the panel carries no fundraising row (AC-3: it moved to the action-icon menu)',
          );
      });
  });
