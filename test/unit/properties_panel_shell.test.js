...
window.eval(sectionUtilsContent);
  ...
  it('should position the properties panel above the filters', function() {
    window.createControls();
    const controls = document.getElementById('print-enhance-controls');
    // ui_ux_overhaul_20260908 Phase 3: the panel is organized into bands
    // (PROPERTIES / CANVAS FILTERS); properties still sits above filters.
    const bands = Array.from(controls.querySelectorAll('.be-ctl-band'));
    const propPanelBand = bands.findIndex((b) =>
      b.querySelector('#print-enhance-properties-panel'),
    );
    const filtersBand = bands.findIndex((b) =>
      b.querySelector('.be-filters-container'),
    );
    assert.ok(propPanelBand !== -1, 'Prop panel band not found');
    assert.ok(filtersBand !== -1, 'Filters band not found');
    assert.ok(propPanelBand < filtersBand, 'Prop panel should be above filters');
  });
});
